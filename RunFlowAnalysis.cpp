...
void readConfig(const TString& _strFileName)
{
  ...
  QCUMULANTGENERICFRAMWORK = env.GetValue("QCUMULANTGENERICFRAMWORK", 0);
...
void RunFlowAnalysis(TString inputFileName, TString outputFileName, TString configFileName = "")
{
  ...
  if (debug)
  {
    ...
    cout << "QCUMULANTGENERICFRAMWORK = " << QCUMULANTGENERICFRAMWORK << endl;
  ...
  for (Int_t iEv = 0; iEv < n_entries; iEv++)
  {
    ...
    { // Zero Track loop for Product LYZ
      for (Int_t iTrk = 0; iTrk < mult; iTrk++)
      {
        ...
        // if (!Acceptance(phi, rand)) continue;
    ...
    { // First Track loop
      ...
      if (pt > minptRF && pt < maxptRF) if (MCEP) flowMCEP->ProcessFirstTrackLoop(eta, phi, 1.);
      // if (!Acceptance(phi, rand)) continue;
      // ipt = findBin(pt);
      if (pt > minptRF && pt < maxptRF)
...
int main(int argc, char **argv)
{
  ...
  if (argc < 5)
  {
    std::cerr << "./RunFlowAnalysis -i INPUT -o OUTPUT [OPTIONAL: -config qcumulant.cfg]" << std::endl;
    return 1;
  }
