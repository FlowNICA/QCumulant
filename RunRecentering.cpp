...
void RunRecentering(TString inputFileName, TString outputFileName, TString configFileName = "")
{
  TStopwatch timer;
  timer.Start();
  ...
  for (Int_t iEv = 0; iEv < n_entries; iEv++)
  {
    ...
    if (qVec[0]->GetMult() != 0 && qVec[1]->GetMult() != 0)
    {
      for (Int_t ieta(0); ieta<neta; ieta++)
      {
        pQx[ieta]->Fill(icent, qVec[ieta]->X());
        pQy[ieta]->Fill(icent, qVec[ieta]->Y());
      }
    }
...
int main(int argc, char **argv)
{
  ...
  if (argc < 5)
  {
    std::cerr << "./Recentering -i INPUT -o OUTPUT [OPTIONAL: -config qcumulant.cfg]" << std::endl;
    return 1;
  }
  ...
  RunRecentering(iFileName, oFileName, configFileName);
