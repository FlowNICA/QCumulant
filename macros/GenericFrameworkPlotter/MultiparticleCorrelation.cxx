...
MultiparticleCorrelation::MultiparticleCorrelation(TProfile *const &pCor2, TProfile *const &pCor4, TProfile *const &pCor6, 
...
{  // 2,4,6-particle correlations of POI (statistical uncertainties calculated according to Cochran)
  ...
  cor2 = term(pCor2,pCor2Var,fCentBin);
  cor4 = term(pCor4,pCor4Var,fCentBin);
  cor6 = term(pCor6,pCor6Var,fCentBin);
  v22Ref = sqrt(cor2.mVal);
  v22Refe = sqrt(1./(4.*cor2.mVal)*cor2.mMSE);
  ...
  cov26 = Covariance(pCov26,pCor2,pCor6,fCentBin,fCentBin,fCentBin);
  cov46 = Covariance(pCov46,pCor4,pCor6,fCentBin,fCentBin,fCentBin);
  v26Ref = pow(2,-1./3.)*pow(cor6.mVal-9*cor2.mVal*cor4.mVal+12*pow(cor2.mVal,3),1./6.);
  v26Refe = sqrt(1./(2*pow(2,4)*pow(v26Ref,10))*(9./2.*pow(4*pow(cor2.mVal,2)-cor4.mVal,2)*cor2.mMSE
  ...
  ratioV24OverV22Refe = sqrt( pow( cor4.mVal/(2.*pow(cor2.mVal, 3.)*pow( 2 - cor4.mVal/pow(cor2.mVal, 2.), 0.75)), 2.) * cor2.mMSE
                            + pow( 1.       /(4.*pow(cor2.mVal, 2.)*pow( 2 - cor4.mVal/pow(cor2.mVal, 2.), 0.75)), 2.) * cor4.mMSE
                            - 2.*cor4.mVal/(8.*pow(cor2.mVal, 5.) * pow( 2 - cor4.mVal/pow(cor2.mVal, 2.), 9./16.)) * cov24);
  ratioV26OverV24Ref = v26Ref / v24Ref;
  ratioV26OverV24Refe = 0; // not calculated yets
  // Differential flow
  cor2red = term(pCor2red,pCor2redVar,fPtBin);
  cor4red = term(pCor4red,pCor4redVar,fPtBin);
  cor6red = term(pCor6red,pCor6redVar,fPtBin);
  ...
  cov26prime = Covariance(pCov26prime,pCor2,pCor6red,fPtBin,fCentBin,fPtBin);
  cov2prime6 = Covariance(pCov2prime6,pCor2red,pCor6,fPtBin,fPtBin,fCentBin);
  cov2prime6prime = Covariance(pCov2prime6prime,pCor2red,pCor6red,fPtBin,fPtBin,fPtBin);
  cov46prime = Covariance(pCov46prime,pCor4,pCor6red,fPtBin,fCentBin,fPtBin);
  cov4prime6 = Covariance(pCov4prime6,pCor4red,pCor6,fPtBin,fPtBin,fCentBin);
  cov4prime6prime = Covariance(pCov4prime6prime,pCor4red,pCor6red,fPtBin,fPtBin,fPtBin);
  cov66prime = Covariance(pCov66prime,pCor6,pCor6red,fPtBin,fCentBin,fPtBin);

  Double_t dNumeratorDiffV26 = cor6red.mVal-9*cor2.mVal*cor4red.mVal+12*cor2red.mVal*pow(cor2.mVal,2);
  v26Dif = dNumeratorDiffV26 / (4.*pow(v26Ref,5));
  Double_t dFourTimesV26RefToTheSixthPower = 4.*pow(v26Ref,6.);
  // Double_t dFourTimesV26RefToTheSixthPower = cor6.mVal-9.*cor2.mVal*cor4.mVal+12.*pow(cor2.mVal,3);
  Double_t dPartialDerivativeOfDiffV26WRT2Corr    = (5.*pow(4.,1./3.)*(9.*cor4.mVal-36.*pow(cor2.mVal,2.))*dNumeratorDiffV26/(12.*pow(dFourTimesV26RefToTheSixthPower,11./6.)))
                                                  - (pow(4.,1./3.)*(9.*cor4red.mVal-24.*cor2.mVal*cor2red.mVal)/(2.*pow(dFourTimesV26RefToTheSixthPower,5./6.)));
  Double_t dPartialDerivativeOfDiffV26WRT2CorrRed = 6.*pow(4.,1./3.)*pow(cor2.mVal,2.)/pow(dFourTimesV26RefToTheSixthPower,5./6.);
  Double_t dPartialDerivativeOfDiffV26WRT4Corr    = 15.*pow(4.,1./3.)*cor2.mVal*dNumeratorDiffV26/(4.*pow(dFourTimesV26RefToTheSixthPower,11./6.));
  Double_t dPartialDerivativeOfDiffV26WRT4CorrRed = (-1.)*9.*pow(4.,1./3.)*cor2.mVal/(2.*pow(dFourTimesV26RefToTheSixthPower,5./6.));
  Double_t dPartialDerivativeOfDiffV26WRT6Corr    = (-1.)*5.*pow(4.,1./3.)*dNumeratorDiffV26/(12.*pow(dFourTimesV26RefToTheSixthPower,11./6.));
  Double_t dPartialDerivativeOfDiffV26WRT6CorrRed = pow(4.,1./3.)/(2.*pow(dFourTimesV26RefToTheSixthPower,5./6.));
  v26Dife = sqrt(
  ...
  ratioV26OverV24Dif = v26Dif / v24Dif;
  ratioV26OverV24Dife = 0; // not calculated yet
}
...
{  // 2,4,6-particle correlations of RFP (statistical uncertainties calculated according to Cochran)
