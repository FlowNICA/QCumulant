...
void PlotV2QCumulantWithAcceptanceCorrection(TString inFileName = "../build/BES_27_new.root", TString outFileName = "graphs_v2QC_AC.root"){
  ...
  //
    ...
    int drawRatioToMethod = 0; // Ratio to v2{2,2-sub}
    ...
    const double maxpt = 3.6;       // for v2 vs pt
    const double minpt = -0.1;        // for v2 vs pt
    const double minV2dif = -0.01;  // for v2 vs pt
    const double maxV2dif = 0.3;    // for v2 vs pt
    // Range of ratio plots
    vector<pair<Double_t,Double_t>> rangeRatio = {{0.79,1.21},{0.79,1.21}};  // range of ratio pads for pT-differential flow plots
  ...
  {
    pNonIsotropicTermRF[i]       = (TProfile*) inFile->FindObjectAny(Form("pNonIsotropicTermRF_%i",i));
    pNonIsotropicTermRFEtaGap[i] = (TProfile*) inFile->FindObjectAny(Form("pNonIsotropicTermRFEtaGap_%i",i));
  }
  ...
  { 
    ...
    for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
    {
      pNonIsotropicTermPOI[i][j]        = (TProfile2D*) inFile->FindObjectAny(Form("pNonIsotropicTermPOI_%i_%i",i,j));
      pNonIsotropicTermPOIEtaGap[i][j]  = (TProfile2D*) inFile->FindObjectAny(Form("pNonIsotropicTermPOIEtaGap_%i_%i",i,j));
    }
  }
  ...
  TProfile *pNonIsotropicTermPOI_cent[npid][ncent][nNonIsotropicTermPOI];
  TProfile *pNonIsotropicTermPOIEtaGap_cent[npid][ncent][nNonIsotropicTermPOI];
  ...
  for (int i = 0; i < npid; i++)
  {
    for (int c = 0; c < ncent; c++)
    {
      ...
      for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
      {
        pNonIsotropicTermPOI_cent[i][c][j]        = (TProfile*) pNonIsotropicTermPOI[i][j]        ->ProfileX(Form("%s_cent%i",pNonIsotropicTermPOI[i][j]      ->GetName(),c), c+1, c+1);
        pNonIsotropicTermPOIEtaGap_cent[i][c][j]  = (TProfile*) pNonIsotropicTermPOIEtaGap[i][j]  ->ProfileX(Form("%s_cent%i",pNonIsotropicTermPOIEtaGap[i][j]->GetName(),c), c+1, c+1);
      }
  ...
  //     for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
  //     {
  //       pNonIsotropicTermPOI_cent[i][c][j]        = (TProfile*) pNonIsotropicTermPOI_cent[i-8][c][j]->Clone();
  //       pNonIsotropicTermPOIEtaGap_cent[i][c][j]  = (TProfile*) pNonIsotropicTermPOIEtaGap_cent[i-8][c][j]->Clone();
  //     }
  ...
  //     for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
  //     {
  //       pNonIsotropicTermPOI_cent[i][c][j]       -> Add(pNonIsotropicTermPOI_cent[i-4][c][j]);
  //       pNonIsotropicTermPOIEtaGap_cent[i][c][j] -> Add(pNonIsotropicTermPOIEtaGap_cent[i-4][c][j]);
  //     }
  //   }
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    // Correction terms
    term correctionTermRF[nNonIsotropicTermRF];
    term correctionTermRFGap[nNonIsotropicTermRF];
    Double_t dNonIsotropicTermRF[nNonIsotropicTermRF];
    Double_t dNonIsotropicTermRFGap[nNonIsotropicTermRF];

    for (Int_t i=0;i<nNonIsotropicTermRF;i++)
    {
      correctionTermRF[i] = term(pNonIsotropicTermRF[i],icent);
      correctionTermRFGap[i] = term(pNonIsotropicTermRFEtaGap[i],icent);
      dNonIsotropicTermRF[i] = correctionTermRF[i].mVal;
      dNonIsotropicTermRFGap[i] = correctionTermRFGap[i].mVal;
    }
    // 2QC
    term cor2 = term(pCorrelator2,icent);
    double v22 = sqrt(cor2.mVal - pow(dNonIsotropicTermRF[C2], 2.) - pow(dNonIsotropicTermRF[C3], 2.));
    ...
    double v24 = pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal
                          - 4. * dNonIsotropicTermRF[C2] * dNonIsotropicTermRF[C9]
                          + 4. * dNonIsotropicTermRF[C3] * dNonIsotropicTermRF[C10]
                          - pow(dNonIsotropicTermRF[C7],2) - pow(dNonIsotropicTermRF[C8],2)
                          + 4. * dNonIsotropicTermRF[C7]
                          * (pow(dNonIsotropicTermRF[C2],2) - pow(dNonIsotropicTermRF[C3],2))
                          + 8. * dNonIsotropicTermRF[C8] * dNonIsotropicTermRF[C3] * dNonIsotropicTermRF[C2]
                          + 8. * cor2.mVal
                          * (pow(dNonIsotropicTermRF[C2],2) + pow(dNonIsotropicTermRF[C3],2))
                          - 6. * pow(pow(dNonIsotropicTermRF[C2],2) + pow(dNonIsotropicTermRF[C3],2),2)),0.25);
    ...
    double v22Gap = sqrt(cor2Gap.mVal - pow(dNonIsotropicTermRFGap[C2], 2.) - pow(dNonIsotropicTermRFGap[C3], 2.));
    ...
    double v24Gap = pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal
                            - 4. * dNonIsotropicTermRFGap[C2] * dNonIsotropicTermRFGap[C9]
                            + 4. * dNonIsotropicTermRFGap[C3] * dNonIsotropicTermRFGap[C10]
                            - pow(dNonIsotropicTermRFGap[C7],2) - pow(dNonIsotropicTermRFGap[C8],2)
                            + 4. * dNonIsotropicTermRFGap[C7]
                            * (pow(dNonIsotropicTermRFGap[C2],2) - pow(dNonIsotropicTermRFGap[C3],2))
                            + 8. * dNonIsotropicTermRFGap[C8] * dNonIsotropicTermRFGap[C3] * dNonIsotropicTermRFGap[C2]
                            + 8. * cor2Gap.mVal
                            * (pow(dNonIsotropicTermRFGap[C2],2) + pow(dNonIsotropicTermRFGap[C3],2))
                            - 6. * pow(pow(dNonIsotropicTermRFGap[C2],2) + pow(dNonIsotropicTermRFGap[C3],2),2)),0.25);
    ...
    for (int id=0;id<npid;id++){ // Differential flow calculation
      for(int ipt=0; ipt<npt; ipt++){ // loop for all pT bin
        ...
        // Correction terms
        term correctionTermPOI[nNonIsotropicTermPOI];
        term correctionTermPOIGap[nNonIsotropicTermPOI];
        Double_t dNonIsotropicTermPOI[nNonIsotropicTermPOI];
        Double_t dNonIsotropicTermPOIGap[nNonIsotropicTermPOI];

        for (Int_t i=0;i<nNonIsotropicTermPOI;i++)
        {
          correctionTermPOI[i] = term(pNonIsotropicTermPOI_cent[id][icent][i],ipt);
          correctionTermPOIGap[i] = term(pNonIsotropicTermPOIEtaGap_cent[id][icent][i],ipt);
          dNonIsotropicTermPOI[i] = correctionTermPOI[i].mVal;
          dNonIsotropicTermPOIGap[i] = correctionTermPOIGap[i].mVal;
        }

        // v22
        term cor2red = term(pReducedCorrelator2_cent[id][icent],ipt);
        double v22Dif = (cor2red.mVal-dNonIsotropicTermPOI[C14]*dNonIsotropicTermRF[C2]-dNonIsotropicTermPOI[C15]*dNonIsotropicTermRF[C3])/v22;
        ...
        double v24Dif = -(-2.*cor2.mVal*cor2red.mVal+cor4red.mVal
                          - dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C9]
                          + dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C10]
                          - dNonIsotropicTermRF[C2] * dNonIsotropicTermPOI[C19a]
                          + dNonIsotropicTermRF[C3] * dNonIsotropicTermPOI[C19b]
                          - 2. * dNonIsotropicTermRF[C2] * dNonIsotropicTermPOI[C18a]
                          - 2. * dNonIsotropicTermRF[C3] * dNonIsotropicTermPOI[C18b]
                          - dNonIsotropicTermPOI[C17a] * dNonIsotropicTermRF[C7]
                          - dNonIsotropicTermPOI[C17b] * dNonIsotropicTermRF[C8]
                          + 2. * dNonIsotropicTermRF[C7]
                          * (dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C2] - dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C3])
                          + 2. * dNonIsotropicTermRF[C8]
                          * (dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C3] + dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C2])
                          + 4. * cor2.mVal
                          * (dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C2] + dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C3])
                          + 2. * dNonIsotropicTermPOI[C17a]
                          * (pow(dNonIsotropicTermRF[C2],2) - pow(dNonIsotropicTermRF[C3],2))
                          + 4. * dNonIsotropicTermPOI[C17b] * dNonIsotropicTermRF[C2] * dNonIsotropicTermRF[C3]
                          + 4. * cor2red.mVal * (pow(dNonIsotropicTermRF[C2],2) + pow(dNonIsotropicTermRF[C3],2))
                          - 6. * (pow(dNonIsotropicTermRF[C2],2) - pow(dNonIsotropicTermRF[C3],2))
                          * (dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C2] - dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C3])
                          - 12. * dNonIsotropicTermRF[C2] * dNonIsotropicTermRF[C3]
                          * (dNonIsotropicTermPOI[C15] * dNonIsotropicTermRF[C2] + dNonIsotropicTermPOI[C14] * dNonIsotropicTermRF[C3]))*pow(v24,-3);
        double ev24Dif = sqrt( pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),-14)
            ...
            * cor2.mMSE
            ...
            + 4*pow(cor2.mVal,2)*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),8)*cor2red.mMSE
            + pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),8)*cor4red.mMSE
            ...
            * cov24
            - 4*cor2.mVal*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),4)
            * (2*cor2.mVal*cor2.mVal*cor2red.mVal-3*cor2.mVal*cor4red.mVal+2*cor4.mVal*cor2red.mVal)
            * cov22prime
            + 2*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),4)
            * (2*cor2.mVal*cor2.mVal*cor2red.mVal-3*cor2.mVal*cor4red.mVal+2*cor4.mVal*cor2red.mVal)
            * cov24prime
            + 3*cor2.mVal*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),4)*(2*cor2.mVal*cor2red.mVal-cor4red.mVal)
            * cov42prime
            - 1.5*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),4)*(2*cor2.mVal*cor2red.mVal-cor4red.mVal)
            * cov44prime
            - 4*cor2.mVal*pow(pow(-1.*(-2.*pow(cor2.mVal,2)+cor4.mVal),0.25),8)*cov2prime4prime));
        v2Dif[1][icent][id][ipt] = v24Dif;
        v2eDif[1][icent][id][ipt] = ev24Dif;
        // v22,subevent
        term cor2redGap = term(pReducedCorrelator2EtaGap_cent[id][icent],ipt);
        double v22DifGap = (cor2redGap.mVal-dNonIsotropicTermPOIGap[C14]*dNonIsotropicTermRFGap[C2]-dNonIsotropicTermPOIGap[C15]*dNonIsotropicTermRFGap[C3])/v22Gap;
        ...
        double v24DifGap = - (-2.*cor2Gap.mVal*cor2redGap.mVal+cor4redGap.mVal
                              - dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C9]
                              + dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C10]
                              - dNonIsotropicTermRFGap[C2] * dNonIsotropicTermPOIGap[C19a]
                              + dNonIsotropicTermRFGap[C3] * dNonIsotropicTermPOIGap[C19b]
                              - 2. * dNonIsotropicTermRFGap[C2] * dNonIsotropicTermPOIGap[C18a]
                              - 2. * dNonIsotropicTermRFGap[C3] * dNonIsotropicTermPOIGap[C18b]
                              - dNonIsotropicTermPOIGap[C17a] * dNonIsotropicTermRFGap[C7]
                              - dNonIsotropicTermPOIGap[C17b] * dNonIsotropicTermRFGap[C8]
                              + 2. * dNonIsotropicTermRFGap[C7]
                              * (dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C2] - dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C3])
                              + 2. * dNonIsotropicTermRFGap[C8]
                              * (dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C3] + dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C2])
                              + 4. * cor2Gap.mVal
                              * (dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C2] + dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C3])
                              + 2. * dNonIsotropicTermPOIGap[C17a]
                              * (pow(dNonIsotropicTermRFGap[C2],2) - pow(dNonIsotropicTermRFGap[C3],2))
                              + 4. * dNonIsotropicTermPOIGap[C17b] * dNonIsotropicTermRFGap[C2] * dNonIsotropicTermRFGap[C3]
                              + 4. * cor2redGap.mVal * (pow(dNonIsotropicTermRFGap[C2],2) + pow(dNonIsotropicTermRFGap[C3],2))
                              - 6. * (pow(dNonIsotropicTermRFGap[C2],2) - pow(dNonIsotropicTermRFGap[C3],2))
                              * (dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C2] - dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C3])
                              - 12. * dNonIsotropicTermRFGap[C2] * dNonIsotropicTermRFGap[C3]
                              * (dNonIsotropicTermPOIGap[C15] * dNonIsotropicTermRFGap[C2] + dNonIsotropicTermPOIGap[C14] * dNonIsotropicTermRFGap[C3]))*pow(v24Gap,-3);
        double ev24DifGap = sqrt( pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),-14)
            ...
            * cor2Gap.mMSE
            ...
            + 4*pow(cor2Gap.mVal,2)*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),8)*cor2redGap.mMSE
            + pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),8)*cor4redGap.mMSE
            ...
            * cov24Gap
            - 4*cor2Gap.mVal*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),4)
            * (2*cor2Gap.mVal*cor2Gap.mVal*cor2redGap.mVal-3*cor2Gap.mVal*cor4redGap.mVal+2*cor4Gap.mVal*cor2redGap.mVal)
            * cov22primeGap
            + 2*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),4)
            * (2*cor2Gap.mVal*cor2Gap.mVal*cor2redGap.mVal-3*cor2Gap.mVal*cor4redGap.mVal+2*cor4Gap.mVal*cor2redGap.mVal)
            * cov24primeGap
            + 3*cor2Gap.mVal*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),4)*(2*cor2Gap.mVal*cor2redGap.mVal-cor4redGap.mVal)
            * cov42primeGap
            - 1.5*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),4)*(2*cor2Gap.mVal*cor2redGap.mVal-cor4redGap.mVal)
            * cov44primeGap
            - 4*cor2Gap.mVal*pow(pow(-1.*(-2.*pow(cor2Gap.mVal,2)+cor4Gap.mVal),0.25),8)*cov2prime4primeGap));
      ...
      for (int i=0; i<nmethod; i++){
        ...
        grDifFl[i][icent][id] -> RemovePoint(0);
        // grDifFl[i][icent][id] -> RemovePoint(0);
  ...
  TProfile *pNonIsotropicTermPOI_PID[npid][nNonIsotropicTermPOI];
  TProfile *pNonIsotropicTermPOIEtaGap_PID[npid][nNonIsotropicTermPOI];
  ...
  for (int i = 0; i < npid; i++)
  {
    ...
    for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
    {
      pNonIsotropicTermPOI_PID[i][j] = (TProfile*) pNonIsotropicTermPOI[i][j]->ProfileY(Form("%s_ptcut_%1.1f_%1.1f",pNonIsotropicTermPOI[i][j]->GetName(),pTBin[binMinPtRFP], pTBin[binMaxPtRFP]), binMinPtRFP+1, binMaxPtRFP);
      pNonIsotropicTermPOIEtaGap_PID[i][j] = (TProfile*) pNonIsotropicTermPOIEtaGap[i][j]->ProfileY(Form("%s_ptcut_%1.1f_%1.1f",pNonIsotropicTermPOIEtaGap[i][j]->GetName(),pTBin[binMinPtRFP], pTBin[binMaxPtRFP]), binMinPtRFP+1, binMaxPtRFP);      
    }
  }
  // for (int i=8;i<npid;i++)
  // {
  ...
  //   for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
  //   {
  //     pNonIsotropicTermPOI_PID[i][j]        = (TProfile*) pNonIsotropicTermPOI_PID[i-8][j]->Clone();
  //     pNonIsotropicTermPOIEtaGap_PID[i][j]  = (TProfile*) pNonIsotropicTermPOIEtaGap_PID[i-8][j]->Clone();
  //   }
  ...
  //   for (Int_t j(0);j<nNonIsotropicTermPOI;j++)
  //   {
  //     pNonIsotropicTermPOI_PID[i][j]        -> Add(pNonIsotropicTermPOI_PID[i-4][j]);
  //     pNonIsotropicTermPOIEtaGap_PID[i][j]  -> Add(pNonIsotropicTermPOIEtaGap_PID[i-4][j]);
  //   }
  // }
  ...
  for (int icent=0; icent<ncent; icent++){ // loop over centrality classes
    ...
    for (int id=0;id<npid;id++){
      ...
      for (Int_t i=0;i<nNonIsotropicTermPOI;i++)
      {
        correctionTermPOI[i] = term(pNonIsotropicTermPOI_PID[id][i],icent);
        correctionTermPOIGap[i] = term(pNonIsotropicTermPOIEtaGap_PID[id][i],icent);
        dNonIsotropicTermPOI[i] = correctionTermPOI[i].mVal;
        dNonIsotropicTermPOIGap[i] = correctionTermPOIGap[i].mVal;
      }
